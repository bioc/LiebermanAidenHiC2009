...
\fixme{It would probably better to move this section to the end of the lab, as an appendix
       that explains how the data object HiC_GM_chr14.RData was created; and to start the main
       text with Section~\ref{sec:adjmat}}.
...
load(file.path("/g/huber/course/LiebermanAidenHiC-Datasets",  "HiC_GM_chr14.RData"))
head(HiC_GM_chr14)
