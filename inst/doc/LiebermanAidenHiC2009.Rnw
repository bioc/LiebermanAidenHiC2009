...
load(file.path("..", "extdata", "HiC_GM_chr14.RData"))
...
with(normDen, image(x=x1, y=x2, z=fhat, 
      col=colorRampPalette(c("white","blue"))(256), useRaster=TRUE))
@
...
with(corDen, image(x=x1,y=x2,z=fhat, 
     zlim=c(-1,1),
...
To compare with the ChIP-seq data, we load the files and store them as \Rclass{Rle} vectors. 
...
\end{center}
We are only interested in data for chromosome 14. For each peak, we want the start position, end position and the signal value in this range. 
...
These are the data files \fixme{Move this file import to Section~\ref{sec:obtain}, explain where you found these files (URL),
        what they contain. Here, we can just load them using \Rfunction{data}.}
<<ChIPfiles>>=
inDir = "/g/huber/course"
inDir = "/Users/huber/LiebermanAiden"
